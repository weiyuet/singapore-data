...
#### Load data ####
raw_prices <- read_csv("data/retail-prices-consumer-items/M212951.csv", skip = 10)
...
tidy_prices %>% 
  ...
  facet_wrap(~item,
             scales = "free_y",
             labeller = labeller(item = label_wrap_gen(width = 25))) +
...
tidy_prices %>% 
  ...
  labs(x = "",
       y = "",
       title = "Price Trends of Common Groceries",
       caption = "Data: Singapore Department of Statistics | Graphic: @weiyuet") +
  theme_bw()
...
#### Load data ####
raw_household_expenditure <- read_csv("data//retail-prices-consumer-items/M212981.csv", skip = 10)
...
tidy_household_expenditure %>%
  ...
  labs(x = "",
       y = "",
       fill = "",
       title = "Average Monthly Household Expenditure - Surveyed Quinquenially",
       subtitle = "There was spiked increase after the 2008 Global Financial Crisis",
       caption = "Data: Singapore Department of Statistics | Graphic: @weiyuet") +
  theme_bw() +
